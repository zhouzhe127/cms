...
import { watch, onMounted, onUnmounted, ref, defineEmits, defineExpose } from 'vue'
...
watch(() => props.previewStyle, (newValue: any) => {
  editor.changePreviewStyle(newValue)
})
watch(() => props.height, (newValue: any) => {
...
defineExpose({ invoke, getRootElement })
</script>
<script lang="ts">
const editorEvents = [
  'load',
...
.tfrEditor {
  width: 100%;
  ::v-global(.toastui-editor-defaultUI) {
    border-radius: 8px !important;
    overflow: hidden;
  }
  ::v-global(.toastui-editor-mode-switch) {
